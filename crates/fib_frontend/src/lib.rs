...
pub unsafe extern "C" fn fib_new_table(net: *mut net, id: u32) -> *mut fib_table {
    ...
    hlist_add_head_rcu(&mut (*tb).tb_hlist, &mut (*net).ipv4.fib_table_hash[h as usize]);
...
#[no_mangle]
pub unsafe extern "C" fn inet_addr_type_dev_table(net: *mut net, dev: *mut c_void, addr: u32) -> u32 {
    let rt_table = l3mdev_fib_table(dev).unwrap_or(RT_TABLE_LOCAL);
...
unsafe fn __inet_dev_addr_type(net: *mut net, dev: *mut c_void, addr: u32, tb_id: u32) -> u32 {
    ...
    if !table.is_null() {
        ...
        if fib_table_lookup(table, &mut flowi4 {
            daddr: addr,
            saddr: 0,
            flowi4_tos: 0,
            flowi4_scope: 0,
...
unsafe fn fib_table_lookup(table: *mut fib_table, fl4: *mut flowi4, res: *mut fib_result, flags: c_int) -> c_int {
...
mod tests {
    ...
    fn test_fib_new_table() {
        ...
        let mut net = net {
            ipv4: ipv4_net {
                fib_table_hash: [hlist_head { first: ptr::null_mut() }; FIB_TABLE_HASHSZ as usize],
