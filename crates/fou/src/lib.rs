...
#[repr(C)]
pub struct sk_buff {
    // Opaque structure - actual fields would be defined in the kernel
    _private: [u8; 0],
}
...
pub unsafe extern "C" fn fou_recv_pull(
    ...
    if fou.family == 0x02 { // AF_INET
        ...
        unsafe { (*ip).tot_len = ((ntohs(tot_len) - len) as u16).to_be() };
        ...
        unsafe { (*ipv6).payload_len = ((ntohs(payload_len) - len) as u16).to_be() };
    }
...
pub unsafe extern "C" fn gue_remcsum(
    ...
    let new_guehdr = unsafe { &mut *( &udp_hdr(skb) as *const udphdr as *mut guehdr ).offset(1) } as *mut guehdr;
