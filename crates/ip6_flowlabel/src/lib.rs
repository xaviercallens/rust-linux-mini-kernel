...
static mut FL_HT: [*mut Ip6Flowlabel; FL_HASH_MASK as usize + 1] = [ptr::null_mut(); FL_HASH_MASK as usize + 1];
...
pub unsafe extern "C" fn fl_free_rcu(head: *mut RcuHead) {
    ...
    if (*fl).share == 1 { // Assuming IPV6_FL_S_PROCESS
        put_pid((*fl).owner);
    }
...
pub unsafe extern "C" fn fl_release(fl: *mut Ip6Flowlabel) {
    ...
    if atomic_dec_and_test(&(*fl).users) {
        ...
        if (*fl).opt.is_some() && (*fl).share == 1 { // Assuming IPV6_FL_S_EXCL
            let opt = (*fl).opt;
            (*fl).opt = ptr::null_mut();
            kfree(opt);
        }
        
        if !timer_pending(&mut IP6_FL_GC_TIMER) || 
...
#[no_mangle]
pub unsafe extern "C" fn fl_intern(net: *mut Net, fl: *mut Ip6Flowlabel, label: u32) -> *mut Ip6Flowlabel {
...
#[no_mangle]
unsafe extern "C" fn net_eq(a: *mut Net, b: *mut Net) -> bool { a == b }
...
unsafe extern "C" fn atomic_inc_not_zero(a: *mut AtomicUsize) -> bool {
    ...
    if val == 0 { false } else {
        (*a).compare_exchange(val, val + 1, Ordering::Relaxed, Ordering::Relaxed).is_ok();
        true
    }
}
...
#[no_mangle]
unsafe extern "C" fn inet6_sk(sk: *mut Sock) -> *mut c_void { sk }

#[no_mangle]
unsafe extern "C" fn timer_pending(timer: *mut TimerList) -> bool { false }

#[no_mangle]
unsafe extern "C" fn time_after(a: c_ulong, b: c_ulong) -> bool { a > b }
